...
class CalculationService {
  ...
  calculateDailyEarnings(workEntry, settings) {
    ...
    console.log(`[CalculationService] 🔍 DEBUG: Analizzando workEntry per calcolo dettagliato:`, {
    ...
    if (useDetailedCalculation) {
      console.log(`[CalculationService] ⭐ Applicando calcolo dettagliato fasce orarie per turni multipli`);
      
      // Calcola ore totali per determinare se è straordinario
      const totalHours = workHours + travelHours;
      const isOvertimePeriod = totalHours > 8;
      
      let totalWorkCalculation = {
        totalHours: 0,
        totalEarnings: 0,
        segments: []
      };
      
      // Calcola primo turno se presente
      if (workEntry.workStart1 && workEntry.workEnd1) {
        const shift1Calculation = this.calculateHourlyRatesByTimeSlots(
        ...
        totalWorkCalculation.totalHours += shift1Calculation.totalHours;
        totalWorkCalculation.totalEarnings += shift1Calculation.totalEarnings;
        totalWorkCalculation.segments.push({
          period: `${workEntry.workStart1}-${workEntry.workEnd1}`,
          ...shift1Calculation
        });
        console.log(`[CalculationService] ✅ Turno 1 (${workEntry.workStart1}-${workEntry.workEnd1}):`, shift1Calculation);
      }
      
      // Calcola secondo turno se presente
      if (workEntry.workStart2 && workEntry.workEnd2) {
        const shift2Calculation = this.calculateHourlyRatesByTimeSlots(
        ...
        totalWorkCalculation.totalHours += shift2Calculation.totalHours;
        totalWorkCalculation.totalEarnings += shift2Calculation.totalEarnings;
        totalWorkCalculation.segments.push({
          period: `${workEntry.workStart2}-${workEntry.workEnd2}`,
          ...shift2Calculation
        });
        console.log(`[CalculationService] ✅ Turno 2 (${workEntry.workStart2}-${workEntry.workEnd2}):`, shift2Calculation);
      }
      
      console.log(`[CalculationService] 📊 Calcolo lavoro fasce orarie TOTALE:`, totalWorkCalculation);
      
      // Imposta le retribuzioni calcolate
      if (isOvertimePeriod) {
        overtimePay = totalWorkCalculation.totalEarnings;
        overtimeHours = totalWorkCalculation.totalHours;
        regularPay = 0;
        regularHours = 0;
      } else {
        regularPay = totalWorkCalculation.totalEarnings;
        regularHours = totalWorkCalculation.totalHours;
      }
    ...
    }  // Fine blocco else (logica standard)
  ...
  calculateEarningsBreakdown(workEntry, settings) {
    ...
    // NUOVO: Verifica se usare calcolo dettagliato per fasce orarie
    console.log(`[CalculationService] 🔍 DEBUG: Analizzando workEntry per calcolo dettagliato (calculateEarningsBreakdown):`, {
      workStart1: workEntry.workStart1,
      workEnd1: workEntry.workEnd1,
      workStart2: workEntry.workStart2,
      workEnd2: workEntry.workEnd2,
    ...
    console.log(`[CalculationService] Modalità calcolo (calculateEarningsBreakdown): ${useDetailedCalculation ? 'FASCE ORARIE DETTAGLIATE' : 'STANDARD'}`, {
      data: workEntry.date,
      workHours,
      travelHours
    });
    ...
    // NUOVO: Calcolo dettagliato per fasce orarie CCNL
    if (useDetailedCalculation) {
      console.log(`[CalculationService] ⭐ Applicando calcolo dettagliato fasce orarie per turni multipli (calculateEarningsBreakdown)`);
      ...
      console.log(`[CalculationService] 📊 Calcolo lavoro fasce orarie TOTALE (calculateEarningsBreakdown):`, totalWorkCalculation);
      
      // Adatta il risultato alla struttura di calculateEarningsBreakdown
      if (isOvertimePeriod) {
        // Tutto straordinario
        result.ordinary.earnings.lavoro_extra = totalWorkCalculation.totalEarnings;
        result.ordinary.hours.lavoro_extra = totalWorkCalculation.totalHours;
        result.ordinary.earnings.giornaliera = 0;
        result.ordinary.hours.lavoro_giornaliera = 0;
      } else {
        // Tutto ordinario
        result.ordinary.earnings.giornaliera = totalWorkCalculation.totalEarnings;
        result.ordinary.hours.lavoro_giornaliera = totalWorkCalculation.totalHours;
        result.ordinary.earnings.lavoro_extra = 0;
        result.ordinary.hours.lavoro_extra = 0;
      }
      
      // Calcola viaggio separatamente
      const travelRate = baseRate * (settings.travelCompensationRate || 1);
      result.ordinary.earnings.viaggio_giornaliera = Math.min(travelHours, Math.max(0, standardWorkDay - workHours)) * travelRate;
      result.ordinary.earnings.viaggio_extra = Math.max(0, travelHours - Math.max(0, standardWorkDay - workHours)) * travelRate;
      
      // Ricalcola totale
      result.ordinary.total = result.ordinary.earnings.giornaliera + 
                             result.ordinary.earnings.viaggio_giornaliera +
                             result.ordinary.earnings.lavoro_extra +
                             result.ordinary.earnings.viaggio_extra;
      
      // Aggiorna i dettagli
      result.details.corrected = true;
      result.details.calculationMethod = 'DETAILED_TIME_SLOTS';
    ...
    }  // Fine blocco else (logica standard calculateEarningsBreakdown)
  ...
  // NUOVO: Calcola la retribuzione per fasce orarie CCNL dettagliate
  ...
  // NUOVO: Determina se usare il calcolo dettagliato per fasce orarie
  shouldUseDetailedTimeSlotCalculation(workEntry) {
    // Estrai orari di inizio e fine dal workEntry
    let startTime = null;
    let endTime = null;
    
    // Prova prima workStart1/workEnd1
    if (workEntry.workStart1) {
      startTime = workEntry.workStart1;
    }
    
    // Usa workEnd2 se presente, altrimenti workEnd1
    if (workEntry.workEnd2) {
      endTime = workEntry.workEnd2;
    } else if (workEntry.workEnd1) {
      endTime = workEntry.workEnd1;
    }
    
    // Se non abbiamo orari specifici, non usare calcolo dettagliato
    ...
    console.log(`[CalculationService] shouldUseDetailedTimeSlotCalculation:`, {
      startTime,
      endTime,
      startHour,
      endHour,
