...
const TimeEntryForm = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (isEdit && entryToEdit) {
      ...
      setForm(prev => ({
        ...
        trasfertaManualOverride: entryToEdit.trasfertaManualOverride || false,
  ...
  const toggleTrasferta = () => {
    ...
    const isHoliday = false; // Semplificazione, idealmente verificheresti con una funzione
    ...
    if (isSpecialDay && !canApplyOnSpecialDays && !form.trasferta) {
      Alert.alert(
        "Avviso",
      ...
      setForm({ 
        ...
        trasfertaManualOverride: isSpecialDay && !form.trasferta // Setta override solo se stiamo attivando in un giorno speciale
      });
    }
  };
  ...
  useEffect(() => {
    ...
    if (!viaggiValidi) {
      ...
      if (isSpecialDay && !canApplyOnSpecialDays) {
