...
const TimeEntryScreen = () => {
  ...
  const renderItem = useCallback(({ item }) => {
    if (item.type === 'standby_only') {
      // Card semplificata per solo reperibilità
      return (
        <TouchableOpacity
          style={[styles.detailedCard, styles.standbyOnlyCard]}
        ...
        >
          ...
          <View style={styles.standbyDetails}>
            <Text style={styles.standbyAmount}>
    ...
    return (
      ...
      >
        ...
        {(item.site_name || item.vehicle_driven || item.targaVeicolo) && (
          <DetailSection
          ...
          >
            {workEntry.departureCompany && workEntry.arrivalSite && (
              <DetailRow 
                ...
                duration={(() => {
                  if (!calculationService.calculateTravelHours || !workEntry.departureCompany || !workEntry.arrivalSite) return null;
                  // Calcolo approssimativo durata andata (metà del totale se ci sono andata e ritorno)
                  const totalTravel = calculationService.calculateTravelHours(workEntry);
                  if (workEntry.departureReturn && workEntry.arrivalCompany) {
                    return formatSafeHours(totalTravel / 2);
                  }
                  return formatSafeHours(totalTravel);
                })()}
              />
            )}
            {workEntry.departureReturn && workEntry.arrivalCompany && (
              <DetailRow 
                ...
                duration={(() => {
                  if (!calculationService.calculateTravelHours || !workEntry.departureReturn || !workEntry.arrivalCompany) return null;
                  // Calcolo approssimativo durata ritorno (metà del totale se ci sono andata e ritorno)
                  const totalTravel = calculationService.calculateTravelHours(workEntry);
        ...
        {(workEntry.isStandbyDay === 1 || (workEntry.interventi && workEntry.interventi.length > 0)) && (
          <DetailSection
            title="Reperibilità"
            icon="phone-in-talk"
            iconColor="#4CAF50"
          >
            {workEntry.interventi && workEntry.interventi.length > 0 && (
              <>
...
const styles = StyleSheet.create({
  container: {
    ...
    paddingTop: 0, // Rimosso padding per alzare la visuale
  },
  loadingContainer: {
    flex: 1,
