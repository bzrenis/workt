...
const EarningsSummary = ({ form, settings, isDateInStandbyCalendar, isStandbyCalendarInitialized }) => {
        ...
        <View style={styles.breakdownSection}>
          ...
          {breakdown.allowances.standby > 0 && (
            <View style={styles.breakdownItem}>
              ...
              <Text style={styles.breakdownDetail}>
                {form.reperibilita ? 'Attivata manualmente nel form' : (isStandbyCalendarInitialized && isDateInStandbyCalendar === true) ? 'Attivata dal calendario reperibilità' : 'Attivazione standard'}
              </Text>
            </View>
          )}
...
const TimeEntryForm = ({ route, navigation }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (settings?.standbySettings?.enabled && 
        ...
        console.log(`Data ${dateStr} nel calendario reperibilità: SÌ`);
        setIsDateInStandbyCalendar(true);
      } else {
        console.log(`Data ${dateStr} nel calendario reperibilità: NO`);
        setIsDateInStandbyCalendar(false);
      }
  ...
  }, [form.date, settings?.standbySettings]);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{padding:16}}>
      ...
      <View style={styles.section}>
        ...
        {isStandbyCalendarInitialized && isDateInStandbyCalendar && !form.reperibilita && (
          <View style={{marginVertical: 6, paddingHorizontal: 16}}>
            <Text style={{color: '#f57c00', fontSize: 12}}>
              Questo giorno è segnato come reperibile nel calendario, ma la reperibilità è disattivata manualmente.
            </Text>
          </View>
        )}
        {form.reperibilita && (
          ...
          onPress={async () => {
            try {
              ...
              const entry = {
                ...
                standbyAllowance: form.reperibilita ? 1 : 0, // Assicura che l'indennità venga calcolata
                isStandbyDay: form.reperibilita ? 1 : 0, // Indica che è un giorno di reperibilità
