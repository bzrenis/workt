...
class CalculationService {
  ...
  calculateDailyEarnings(workEntry, settings) {
    ...
    console.log(`[CalculationService] Indennità reperibilità finale per ${workEntry.date}: ${standbyAllowance.toFixed(2)}€ (manuale: ${isManuallyActivated}, disattivata: ${isManuallyDeactivated}, calendario: ${isInCalendar})`);
    ...
    if (travelAllowanceEnabled && travelAllowanceAmount > 0) {
      ...
      if (attiva) {
        ...
        // Applicazione maggiorazioni per tipologia di giorno (come per reperibilità)
        if (isSunday || isPublicHoliday) {
          travelAllowance = baseTravelAllowance * holidayMultiplier * travelAllowancePercent;
          console.log(`[CalculationService] Indennità trasferta ${isSunday ? 'domenica' : 'festivo'} per ${workEntry.date}: ${baseTravelAllowance}€ × ${holidayMultiplier} × ${travelAllowancePercent} = ${travelAllowance.toFixed(2)}€`);
        } else if (isSaturday) {
          travelAllowance = baseTravelAllowance * saturdayMultiplier * travelAllowancePercent;
          console.log(`[CalculationService] Indennità trasferta sabato per ${workEntry.date}: ${baseTravelAllowance}€ × ${saturdayMultiplier} × ${travelAllowancePercent} = ${travelAllowance.toFixed(2)}€`);
        } else {
          travelAllowance = baseTravelAllowance * travelAllowancePercent;
          console.log(`[CalculationService] Indennità trasferta feriale per ${workEntry.date}: ${baseTravelAllowance}€ × ${travelAllowancePercent} = ${travelAllowance.toFixed(2)}€`);
        }
      }
    }
  ...
  calculateAllowances(workEntry, settings, hoursBreakdown) {
      ...
      const isInCalendar = standbySettings.enabled && 
