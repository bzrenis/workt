  ...
  return (
    <View style={styles.summaryContainer}>
      ...
      <View style={styles.totalSection}>
        ...
        {(breakdown.details?.isSunday || breakdown.details?.isHoliday) && (
          <Text style={{fontSize: 12, color: '#2e7d32', fontStyle: 'italic', marginTop: 4}}>
            La retribuzione è calcolata con maggiorazione CCNL per {breakdown.details?.isSunday ? 'domenica' : 'festività'} (+{((settings.contract?.overtimeRates?.holiday || 1.3) - 1) * 100}%) su tutte le ore, senza indennità di trasferta.
          </Text>
        )}
      </View>
    </View>
...
const TimeEntryForm = ({ route, navigation }) => {
  ...
  const toggleTrasferta = () => {
    setForm({ ...form, trasferta: !form.trasferta });
  };
  ...
  useEffect(() => {
    ...
    if (!viaggiValidi) {
      ...
      // Attiva trasferta solo se c'è viaggio
      if (!form.trasferta && (totalTravel > 0)) setForm(f => ({ ...f, trasferta: true }));
      // Salva la percentuale per CalculationService (es: 1.0 o 0.5)
      setForm(f => ({ ...f, trasfertaPercent: giornataIntera ? 1.0 : 0.5 }));
    }
  }, [form.viaggi]);
