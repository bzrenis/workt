  ...
  const loadWorkData = async () => {
    try {
      ...
      if (entries.length > 0) {
        ...
        console.log('🔍 PDF Export: Stats calcolate:', JSON.stringify(stats, null, 2));
        setMonthlyStats(stats);
      } else {
        console.log('🔍 PDF Export: Nessuna entry trovata, stats vuote');
        setMonthlyStats({});
      }
    } catch (error) {
              ...
              try {
                ...
                const formattedEntries = workEntries.map(entry => {
                  if (calculationService && settings) {
                    // 🚀 CRITICAL FIX: Parse entry usando createWorkEntryFromData come fa DashboardScreen
                    ...
                    console.log('🔍 PDF DEBUG - Breakdown result:', JSON.stringify(breakdown, null, 2));
                    ...
                    return {
                      ...
                      // Ore calcolate
                      totalHours: workHours + travelHours + totalStandbyHours,
                      workHours,
                      travelHours,
                      standbyHours: totalStandbyHours,
                      standbyWorkHours,
                      standbyTravelHours,
                      regularHours: breakdown?.workHours?.regular || 0,
                      overtimeHours: breakdown?.workHours?.overtime || 0,
                      nightHours: breakdown?.workHours?.night || 0,
                      ...
                      // Guadagni
                      regularEarnings: breakdown?.earnings?.regular || 0,
                      overtimeEarnings: breakdown?.earnings?.overtime || 0,
                      nightEarnings: breakdown?.earnings?.night || 0,
                      travelEarnings: breakdown?.earnings?.travel || 0,
                      standbyEarnings: breakdown?.earnings?.standby || 0,
