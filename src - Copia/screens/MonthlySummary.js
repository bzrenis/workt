...
const MonthlySummary = ({ month, year }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setIsLoading(true);
        const startDate = new Date(year, month - 1, 1); // mese è 0-based in Date
        const endDate = new Date(year, month, 0);
        
        // Formato ISO per la query
        const start = formatDate(startDate, 'yyyy-MM-dd');
        const end = formatDate(endDate, 'yyyy-MM-dd');
        
        // Carica tutti gli inserimenti orari del mese
        const data = await DatabaseService.getTimeEntries(start, end);
        setEntries(data || []);
      } catch (error) {
