...
const ContractSettingsScreen = ({ navigation }) => {
  ...
  const calculateRates = (monthlySalary, useCustomRates = formData.customRatesEnabled, customDaily = formData.customDailyRate, customHourly = formData.customHourlyRate) => {
    ...
    const overtimeDay = hourlyRate * 1.20; // Straordinario diurno +20%
    const overtimeNightUntil22 = hourlyRate * 1.25; // Straordinario notturno fino alle 22 +25%
    const overtimeNightAfter22 = hourlyRate * 1.35; // Straordinario notturno dopo le 22 +35%
    const overtimeHoliday = hourlyRate * 1.50; // Straordinario festivo +50%
    const overtimeSunday = hourlyRate * 1.50; // Straordinario domenicale +50%
    const ordinaryNight = hourlyRate * 1.25; // Lavoro ordinario notturno +25%
    const ordinaryHoliday = hourlyRate * 1.30; // Lavoro ordinario festivo +30%
    const ordinarySunday = hourlyRate * 1.30; // Lavoro ordinario domenicale +30%
    const ordinaryNightHoliday = hourlyRate * 1.60; // Lavoro ordinario notturno festivo +60%
    
    // Calcoli per maggiorazioni composte reperibilità
    const standbyOrdinarySaturdayNight = hourlyRate * 1.25 * 1.25; // Sabato (25%) + notturno (25%) = +56%
    const standbyOvertimeSaturdayNight = hourlyRate * 1.25 * 1.35; // Sabato (25%) + notturno straord (35%) = +69%
    const standbyOrdinaryHolidayNight = hourlyRate * 1.25 * 1.30; // Notturno (25%) + festivo (30%) = +62%
    const standbyOvertimeHolidayNight = hourlyRate * 1.35 * 1.30; // Notturno straord (35%) + festivo (30%) = +76%
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.formContainer}>
          ...
          <View style={[styles.inputGroup, { backgroundColor: formData.customRatesEnabled ? '#f0f8ff' : 'transparent', padding: formData.customRatesEnabled ? 15 : 0, borderRadius: 8, borderWidth: formData.customRatesEnabled ? 1 : 0, borderColor: '#2196F3' }]}>
            <TouchableOpacity 
        ...
        {calculatedRates && typeof calculatedRates.monthlySalary === 'number' && !isNaN(calculatedRates.monthlySalary) ? (
          <View style={styles.calculatedRatesContainer}>
            <Text style={styles.sectionTitle}>
              ...
              {calculatedRates.isCustomRates && <Text style={{ color: '#2196F3', fontSize: 14 }}> 🎯</Text>}
            </Text>
            <View style={styles.rateCard}>
              ...
              <View style={styles.rateRow}>
                ...
                <Text style={[styles.rateValue, calculatedRates.isCustomRates && { color: '#2196F3', fontWeight: 'bold' }]}>
              ...
              <View style={styles.rateRow}>
                <Text style={[styles.rateLabel, {color: calculatedRates.isCustomRates ? '#2196F3' : '#2196F3'}]}>
                  Retribuzione Oraria {calculatedRates.isCustomRates ? 'Personalizzata' : 'CCNL'}
                </Text>
                <Text style={[styles.rateValue, { color: '#2196F3', fontWeight: calculatedRates.isCustomRates ? 'bold' : 'normal' }]}>
              ...
              {calculatedRates.isCustomRates && (
                <Text style={{fontSize:12, color:'#2196F3', marginBottom:8, marginLeft:4, fontWeight: 'bold'}}>
