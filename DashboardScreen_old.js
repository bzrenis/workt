...
const DetailedEarningsBreakdown = ({ breakdown, expanded, onToggle, settings }) => {
  // Gestione sicurezza: Se breakdown è undefined o null, usa valori vuoti
  if (!breakdown) {
    return (
        ...
        <FadeInCard style={styles.breakdownContent}>
          ...
          {breakdown?.ordinary && (
            <View style={styles.detailBreakdownSection}>
              <Text style={styles.detailBreakdownSubtitle}>Attività Ordinarie</Text>
          ...
          {breakdown?.standby && breakdown.standby.totalEarnings > 0 && (
            <View style={styles.detailBreakdownSection}>
              <Text style={styles.detailBreakdownSubtitle}>Interventi Reperibilità</Text>
              
              {/* Lavoro reperibilità - mostra solo le voci con valori > 0 */}
              ...
              {/* Viaggio reperibilità - mostra solo le voci con valori > 0 */}
              ...
              <View style={[styles.detailBreakdownRow, styles.detailTotalRow]}>
                <Text style={styles.detailBreakdownLabel}>Totale reperibilità</Text>
          ...
          {breakdown?.allowances && (breakdown.allowances.travel > 0 || breakdown.allowances.standby > 0 || breakdown.allowances.meal > 0) && (
            <View style={styles.detailBreakdownSection}>
              <Text style={styles.detailBreakdownSubtitle}>Indennità e Buoni</Text>
              
              {breakdown.allowances.travel > 0 && (
                <View style={styles.detailBreakdownItem}>
                  <View style={styles.detailBreakdownRow}>
                    <Text style={styles.detailBreakdownLabel}>Indennità trasferta</Text>
              ...
              {breakdown.allowances.standby > 0 && (
                <View style={styles.detailBreakdownItem}>
                  <View style={styles.detailBreakdownRow}>
                    <Text style={styles.detailBreakdownLabel}>Indennità reperibilità</Text>
                    <Text style={styles.detailBreakdownValue}>{formatSafeAmount(breakdown.allowances.standby)}</Text>
                  </View>
                  <Text style={styles.detailBreakdownDetail}>
                    CCNL per giorni di reperibilità
                  </Text>
                </View>
              )}
              ...
              <View style={[styles.detailBreakdownRow, styles.detailTotalRow]}>
                <Text style={styles.detailBreakdownLabel}>Totale indennità</Text>
                <Text style={styles.detailBreakdownTotal}>
        ...
        <View style={styles.breakdownContent}>
            ...
            <View style={styles.breakdownSection}>
              ...
              {((breakdown.ordinary.hours?.lavoro_giornaliera || 0) + (breakdown.ordinary.hours?.viaggio_giornaliera || 0)) > 0 && (
                <View style={styles.breakdownItem}>
                  ...
                  <View style={styles.itemDetails}>
                    {breakdown.ordinary.hours.lavoro_giornaliera > 0 && (
                      <Text style={styles.itemDetail}>• Lavoro: {formatSafeHours(breakdown.ordinary.hours.lavoro_giornaliera)}</Text>
                    )}
                    {breakdown.ordinary.hours.viaggio_giornaliera > 0 && (
                      <Text style={styles.itemDetail}>• Viaggio: {formatSafeHours(breakdown.ordinary.hours.viaggio_giornaliera)}</Text>
                    )}
                  </View>
                  {breakdown.ordinary.earnings?.giornaliera > 0 && (
              ...
              {(breakdown.ordinary.hours?.lavoro_extra > 0 || breakdown.ordinary.hours?.viaggio_extra > 0) && (
                <View style={styles.breakdownItem}>
                  ...
                  <View style={styles.itemDetails}>
                    {breakdown.ordinary.hours.lavoro_extra > 0 && (
                      <Text style={styles.itemDetail}>• Lavoro extra: {formatSafeHours(breakdown.ordinary.hours.lavoro_extra)}</Text>
                    )}
                    {breakdown.ordinary.hours.viaggio_extra > 0 && (
                      <Text style={styles.itemDetail}>• Viaggio extra: {formatSafeHours(breakdown.ordinary.hours.viaggio_extra)}</Text>
                    )}
                  </View>
                  {(breakdown.ordinary.earnings?.straordinario_giorno > 0 || 
                    breakdown.ordinary.earnings?.straordinario_notte_22 > 0 || 
          ...
          {breakdown.standby && breakdown.standby.totalEarnings > 0 && (
            <View style={styles.breakdownSection}>
                    ...
                    {Object.entries(breakdown.standby.workHours || {}).map(([type, hours]) => {
                      ...
                      return (
                        <View key={type} style={styles.bonusRow}>
                          <Text style={styles.itemDetail}>• {labels[type] || type}: {formatSafeHours(hours)}</Text>
              ...
              {Object.entries(breakdown.standby.travelHours || {}).some(([_, hours]) => hours > 0) && (
                <View style={styles.breakdownItem}>
                  <Text style={styles.itemLabel}>Viaggi reperibilità</Text>
                  <View style={styles.itemDetails}>
                    {Object.entries(breakdown.standby.travelHours || {}).map(([type, hours]) => {
                      if (hours <= 0) return null;
                      const earnings = breakdown.standby.travelEarnings?.[type] || 0;
              ...
              <View style={styles.sectionTotal}>
                <Text style={styles.sectionTotalLabel}>Totale reperibilità</Text>
          ...
          {breakdown.allowances && 
           (breakdown.allowances.travel > 0 || breakdown.allowances.standby > 0 || breakdown.allowances.meal > 0) && (
            <View style={styles.breakdownSection}>
              <Text style={styles.sectionTitle}>Indennità e Rimborsi</Text>
              
              {breakdown.allowances.travel > 0 && (
                <View style={styles.allowanceRow}>
                  <Text style={styles.allowanceLabel}>Indennità trasferta</Text>
              ...
              {breakdown.allowances.standby > 0 && (
                <View style={styles.allowanceRow}>
                  <Text style={styles.allowanceLabel}>Indennità reperibilità</Text>
              ...
              <View style={styles.sectionTotal}>
                <Text style={styles.sectionTotalLabel}>Totale indennità</Text>
  ...
  const monthlyStats = useMemo(() => {
    ...
    if (!settings || filteredEntries.length === 0) {
      return {
        ...
        // 🔥 NUOVO: Breakdown aggregato vuoto
        aggregatedBreakdown: {
          totalEarnings: 0,
          ordinary: {
            hours: {
    ...
    let stats = {
      ...
      // 🔥 NUOVO: Breakdown aggregato mensile per DetailedEarningsBreakdown
      aggregatedBreakdown: {
        totalEarnings: 0,
        ordinary: {
          hours: {
